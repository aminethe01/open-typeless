...
contextBridge.exposeInMainWorld("api", {
  ...
  session: {
    get: (): Promise<SessionData> =>
      ipcRenderer.invoke(IPC_CHANNELS.SESSION.GET),
...
contextBridge.exposeInMainWorld("api", {
  tabs: {
    onNewTab: (handler: () => void) => {
      ...
      return () =>
        ipcRenderer.removeListener(IPC_CHANNELS.TABS.NEW_TAB, wrapped);
    },
    onCloseActiveTab: (handler: () => void) => {
      const wrapped = () => handler();
      ipcRenderer.on(IPC_CHANNELS.TABS.CLOSE_ACTIVE, wrapped);
      return () =>
        ipcRenderer.removeListener(IPC_CHANNELS.TABS.CLOSE_ACTIVE, wrapped);
    },
  },
});
```
